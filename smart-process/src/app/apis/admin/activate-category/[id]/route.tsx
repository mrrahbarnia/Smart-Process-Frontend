...
const EXTERNAL_API = `${EXTERNAL_BASE_ENDPOINT}/admin/activate-category`;

export const PUT = async (request: NextRequest, {params}: {params: {id: string}}) => {
    const categoryId = params.id;
    try {
        const accessToken = await getToken();
        await axios.put(`${EXTERNAL_API}/${categoryId}/`, {}, {
            headers: {
        ...
        if (axios.isAxiosError(error)) {
            if (error.response?.data && error.response?.data.detail === 'Could not validate credentials' ) {
                ...
                return NextResponse.json({"activated": false}, {status: 403})
            }
        } else {
