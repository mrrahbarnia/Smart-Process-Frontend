...
import useGetActiveProducts from "@/hooks/useQueries/useGetActiveProducts";
...
const ProductsContainer = () => {
    ...
    const {
        ...
        productsData,
        productsIsPending
    } = useGetActiveProducts(searchFilterParams);
    const {
        register,
        handleSubmit,
        setValue,
    ...
    const onSubmit: SubmitHandler<InputTypes> = async(data) => {
        const url = `/products/?nameContain=${data.nameContain}&brandExact=${data.brandExact}&categoryExact=${data.categoryExact}`;
        setShowSearchMenu(false);
        return router.replace(url);
    }

    const previousHandler = () => {
        const url = `/products/?page=${searchParams.get("page") ? Number(searchParams.get("page")) -1  : "1"}${searchParams.get("nameContain") ? `&nameContain=${searchParams.get("nameContain")}` : ""}${searchParams.get("brandExact") ? `&brandExact=${searchParams.get("brandExact")}` : ""}${searchParams.get("categoryExact") ? `&categoryExact=${searchParams.get("categoryExact")}` : ""}`
        return router.replace(url)
    };

    const nextHandler = () => {
        const url = `/products/?page=${searchParams.get("page") ? Number(searchParams.get("page")) + 1  : "2"}${searchParams.get("nameContain") ? `&nameContain=${searchParams.get("nameContain")}` : ""}${searchParams.get("brandExact") ? `&brandExact=${searchParams.get("brandExact")}` : ""}${searchParams.get("categoryExact") ? `&categoryExact=${searchParams.get("categoryExact")}` : ""}`
        return router.replace(url);
    };
