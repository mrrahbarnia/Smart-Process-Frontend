...
import { useDeleteCategory } from "@/hooks/useMutations/useDeleteCategory";
import { useAuthStore } from "@/store/useAuthStore";


const DeleteModal = ({categoryId, closeModalHandler}: {categoryId: number, closeModalHandler: Dispatch<SetStateAction<boolean>>}) => {
    const {deleteIsPending, deleteMutateAsync} = useDeleteCategory();
    const logout = useAuthStore((state) => state.logout);

    const deleteHandler = () => {
        deleteMutateAsync({id: categoryId})
