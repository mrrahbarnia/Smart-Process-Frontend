...
import { useDeleteProduct } from "@/hooks/useMutations/useDeleteProduct";
import { useAuthStore } from "@/store/useAuthStore";


const DeleteModal = ({productId, closeModalHandler}: {productId: string, closeModalHandler: Dispatch<SetStateAction<boolean>>}) => {
    const {deleteIsPending, deleteMutateAsync} = useDeleteProduct();
    const logout = useAuthStore((state) => state.logout);

    const deleteHandler = () => {
        deleteMutateAsync({id: productId})
