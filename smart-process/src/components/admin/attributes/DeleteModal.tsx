...
import { useDeleteAttribute } from "@/hooks/useMutations/useDeleteAttribute";
import { useAuthStore } from "@/store/useAuthStore";


const DeleteModal = ({attributeName, closeModalHandler}: {attributeName: string, closeModalHandler: Dispatch<SetStateAction<boolean>>}) => {
    const {deleteIsPending, deleteMutateAsync} = useDeleteAttribute();
    const logout = useAuthStore((state) => state.logout);

    const deleteHandler = () => {
        deleteMutateAsync({name: attributeName})
