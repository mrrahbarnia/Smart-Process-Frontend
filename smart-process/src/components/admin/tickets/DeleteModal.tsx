...
import { useDeleteTicket } from "@/hooks/useMutations/useDeleteTicket";
import { useAuthStore } from "@/store/useAuthStore";


const DeleteModal = ({ticketId, closeModalHandler}: {ticketId: number, closeModalHandler: Dispatch<SetStateAction<boolean>>}) => {
    const {deleteIsPending, deleteMutateAsync} = useDeleteTicket();
    const logout = useAuthStore((state) => state.logout);

    const deleteHandler = () => {
        deleteMutateAsync({id: ticketId})
