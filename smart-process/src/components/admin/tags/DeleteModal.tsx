...
import { useDeleteTag } from "@/hooks/useMutations/useDeleteTag";
import { useAuthStore } from "@/store/useAuthStore";


const DeleteModal = ({tagName, closeModalHandler}: {tagName: string, closeModalHandler: Dispatch<SetStateAction<boolean>>}) => {
    const {deleteIsPending, deleteMutateAsync} = useDeleteTag();
    const logout = useAuthStore((state) => state.logout);

    const deleteHandler = () => {
        deleteMutateAsync({name: tagName})
