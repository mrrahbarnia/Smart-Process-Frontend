...
import { useDeleteBrand } from "@/hooks/useMutations/useDeleteBrand";
import { useAuthStore } from "@/store/useAuthStore";


const DeleteModal = ({brandSlug, closeModalHandler}: {brandSlug: string, closeModalHandler: Dispatch<SetStateAction<boolean>>}) => {
    const {deleteIsPending, deleteMutateAsync} = useDeleteBrand();
    const logout = useAuthStore((state) => state.logout);

    const deleteHandler = () => {
        deleteMutateAsync({slug: brandSlug})
